...
- **Check the official docs before you answer.** Before answering any EDS or authoring question, ground yourself in the source of truth: the AEM Edge Delivery docs at **https://www.aem.live/**, including the Experience Workspace docs (start at **https://www.aem.live/docs/ew/about**). Don't answer EDS/EW questions from memory alone — confirm against the docs, then answer. (Fast full-text search: `curl -s https://www.aem.live/docpages-index.json | jq -r '.data[] | select(.content | test("KEYWORD";"i")) | "\(.path): \(.title)"'`.) If the docs contradict something you were about to say, trust the docs and say so.
- **Authoring is Experience Workspace — not DA, not the Universal Editor.** Experience Workspace (EW) is Document Authoring upgraded ([Adobe explains](https://www.aem.live/docs/ew/da-is-ew)), so call it EW. It still runs on `da.live` URLs (`da.live/config`, `/.da/…` paths, `?dapreview`) — keep those exactly as they are. The older `docs.da.live` site predates EW; when it disagrees with aem.live, aem.live wins. This project doesn't use the Universal Editor or AEM Sites authoring, so skip docs for those.
